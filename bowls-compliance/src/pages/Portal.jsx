...
export default function Portal() {
  ...
  const s = {
    ...
    header: { background: '#1a1a1a', color: 'white', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 },
    ...
    hero: { background: 'linear-gradient(135deg, #1a1a1a 0%, #333333 100%)', color: 'white', padding: '64px 32px', textAlign: 'center' },
    ...
    btn: { padding: '8px 16px', background: '#1a1a1a', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 },
  ...
  return (
    <div style={s.wrap}>
      <header style={s.header}>
        ...
        <button style={{ background: 'none', border: '1px solid rgba(255,255,255,0.3)', color: 'rgba(255,255,255,0.6)', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12 }} onClick={() => navigate('/login')}>Admin</button>
      </header>
        ...
        <div style={s.hero}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            {navItems.filter(n => n.id !== 'home').map(n => (
              <button key={n.id} onClick={() => setActive(n.id)} style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', color: 'white', padding: '10px 20px', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>
                {n.icon} {n.label}
              </button>
            ))}
          </div>
            ...
            {docCategories.map(cat => {
              ...
              return (
                <div key={cat} style={{ marginBottom: 28 }}>
                  <div style={{ fontWeight: 700, color: '#333333', marginBottom: 12, fontSize: 15 }}>{cat}</div>
                  <div style={s.grid3}>
                    {catDocs.map(doc => (
                      <div key={doc.id} style={s.docCard}>
                        <div style={{ fontSize: 28 }}>📄</div>
                        <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1a1a' }}>{doc.name}</div>
          ...
          <div>
            ...
            {events.map(ev => (
              <div key={ev.id} style={{ ...s.card, display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                <div style={{ background: '#1a1a1a', color: 'white', borderRadius: 12, padding: '12px 16px', textAlign: 'center', minWidth: 60 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 16, color: '#1a1a1a', marginBottom: 4 }}>{ev.title}</div>
                  <div style={{ color: '#64748b', fontSize: 14, marginBottom: 8 }}>{ev.description}</div>
                  <span style={s.tag(ev.type === 'Competition' ? '#555555' : ev.type === 'Birthday' ? '#888888' : ev.type === 'Venue Hire' ? '#444444' : '#222222')}>{ev.type}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div style={s.grid2}>
              {contacts.map(c => (
                <div key={c.id} style={s.card}>
                  <div style={{ fontWeight: 700, fontSize: 16, color: '#1a1a1a', marginBottom: 4 }}>{c.name}</div>
                  <div style={{ color: '#333333', fontWeight: 600, fontSize: 13, marginBottom: 8 }}>{c.role}</div>
          ...
          <div style={s.grid3}>
            {navItems.filter(n => n.id !== 'home').map(n => (
              <div key={n.id} onClick={() => setActive(n.id)} style={{ ...s.card, cursor: 'pointer', textAlign: 'center', padding: 32 }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, color: '#1a1a1a' }}>{n.label}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <footer style={{ background: '#1a1a1a', color: 'rgba(255,255,255,0.5)', textAlign: 'center', padding: '20px 32px', fontSize: 13 }}>
